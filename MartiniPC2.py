...
class MartiniPC2(object):

    def __init__(self, x, y, z, restrain=True, top_gone=False):
        ...
        if restrain:
            poly.restrain_completely(topology_gone=top_gone)

        for i in range(20 * x_num):
            for j in range(20 * y_num):
                current = np.add(np.multiply(i, l_v[0]), np.multiply(j, l_v[1]))
                if current[0] < x and current[1] < y and current[0] > 0 and \
                        current[1] > 0:
                    current = cp.deepcopy(poly)
                    current.align([0, 0, 1])
                    current.shift(np.add(np.multiply(i, l_v[0]), np.multiply(j, l_v[1])))
                    #self.chain_orient(current, 0)
    ...
    def chain_orient(self, chain, direction):
